...
// 23/10/2019
// Change LoadWav to accept const arrays and string literals.
...
// CAUTION - Multiple early returns
{
	...
	// Why does mmioOpen need a modifiable string buffer?
	// Whatever. Make a modifiable string buffer
	size_t length = wcslen(filename);
	wchar_t* filenameBuffer = new wchar_t[length + 1];
	wcscpy_s(filenameBuffer, length+1, filename);

	// Open the wav file
	hWaveFile = mmioOpen(filenameBuffer, NULL, MMIO_READ|MMIO_ALLOCBUF);
	...
	if (!hWaveFile)			// If file could not open
	{
		...
		ErrorLogger::Writeln(filenameBuffer);
